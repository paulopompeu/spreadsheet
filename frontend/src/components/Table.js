...
async function updateMongo(id, tabela) {
  console.log(id)
  console.log(tabela)
  await api.post('/','', {
    "headers" : {
...
async function getFromMongo(id) {
  const getSheet = await api.get(`/${id}`);
  return JSON.stringify(getSheet.data)
}
...
export default class Table extends React.Component {
  ...
  componentWillMount() {
    ...
    const teste = getFromMongo(this.props.id);
    console.log(teste);
  }
  ...
  handleChangedCell = ({ x, y }, value) => {
    ...
    if (this.props.saveToLocalStorage && window && window.localStorage) {
      ...
      updateMongo(this.props.id, JSON.stringify(modifiedData));
    }
  }
